...
export const DeviceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadDevices = useCallback(async () => {
    ...
    try {
      ...
      // Convert to DeviceData format - check for globalSettings before accessing
      const activeDeviceId = store.globalSettings?.activeDeviceId || null;
      const convertedDevices: DeviceData[] = storeDevices.map(device => 
        convertToDeviceData(device, device.id === activeDeviceId)
      );
