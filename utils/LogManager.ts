...
import { LogEntry } from '../types'; // Import from types
import { safeGetItem, safeSetItem } from './storageUtils';

// Unified LogManager class
class LogManager {
    private static readonly LEGACY_LOGS_KEY = 'smsCommandLogs'; // Keep for migration/fallback
    ...
    public static async addLog(
      ...
      try {
        if (deviceId) {
            ...
            const deviceLogsJson = await safeGetItem(deviceLogsKey, '[]');
            const deviceLogs: LogEntry[] = JSON.parse(deviceLogsJson);
            const updatedDeviceLogs = [newLog, ...deviceLogs.slice(0, 199)];
    ...
    // Get logs (device-specific or system)
    ...
    // Clear logs (device-specific or system)
